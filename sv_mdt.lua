...
AddEventHandler("mdt:saveOffenderChanges", function(id, changes, identifier)
	...
	exports.oxmysql:execute('SELECT * FROM `user_mdt` WHERE `char_id` = ?', {id}, function(result)
		if result[1] then
			exports.oxmysql:query('UPDATE `user_mdt` SET `notes` = ?, `mugshot_url` = ?, `bail` = ? WHERE `char_id` = ?', {changes.notes, changes.mugshot_url, changes.bail, id})
		else
		...
		if changes.convictions ~= nil then
			for conviction, amount in pairs(changes.convictions) do	
				exports.oxmysql:query('UPDATE `user_convictions` SET `count` = ? WHERE `char_id` = ? AND `offense` = ?', {id, amount, conviction})
			end
		end

		for i = 1, #changes.convictions_removed do
			exports.oxmysql:query('DELETE FROM `user_convictions` WHERE `char_id` = ? AND `offense` = ?', {id, changes.convictions_removed[i]})
		end
...
AddEventHandler("mdt:saveReportChanges", function(data)
	exports.oxmysql:query('UPDATE `mdt_reports` SET `title` = ?, `incident` = ? WHERE `id` = ?', {data.id, data.title, data.incident})
...
AddEventHandler("mdt:deleteReport", function(id)
	exports.oxmysql:query('DELETE FROM `mdt_reports` WHERE `id` = ?', {id})
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	for offense, count in pairs(data.charges) do
		exports.oxmysql:execute('SELECT * FROM `user_convictions` WHERE `offense` = ? AND `char_id` = ?', {offense, data.char_id}, function(result)
			if result[1] then
				exports.oxmysql:query('UPDATE `user_convictions` SET `count` = ? WHERE `offense` = ? AND `char_id` = ?', {data.char_id, offense, count + 1})
			else
...
AddEventHandler("mdt:deleteWarrant", function(id)
	local usource = source
	exports.oxmysql:query('DELETE FROM `mdt_warrants` WHERE `id` = ?', {id}, function()
		TriggerClientEvent("mdt:completedWarrantAction", usource)
	end)
...
AddEventHandler("mdt:saveVehicleChanges", function(data)
	...
	exports.oxmysql:execute('SELECT * FROM `vehicle_mdt` WHERE `plate` = ?', {data.plate}, function(result)
		if result[1] then
			exports.oxmysql:query('UPDATE `vehicle_mdt` SET `stolen` = ?, `notes` = ? WHERE `plate` = ?', {data.plate, data.stolen, data.notes})
		else
